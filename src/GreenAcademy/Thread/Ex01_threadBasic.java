...
//** run() 메서드
//=> GreenAcademy.thread 실행의 주체 메서드
//   생성된 GreenAcademy.thread 의 main 메서드
//=> GreenAcademy.thread 를 통해 실행하려는 기능 (객체가 해야되는 일) 을 여기에 작성함.
...
public class Ex01_threadBasic {
	...
	public static void main(String[] args) {
		...
		// => 1.2) 실행
		// => start() 호출 : GreenAcademy.thread Start -> run() 메서드 실행
//		t01.start();
//		t02.start();
		
		// => 비교 : run() 호출
		// => multi GreenAcademy.thread 는 실행되지 않고 main 이 일반 메서드 호출 & 실행.
		//    GreenAcademy.thread 적용없이 순서대로 실행
//		t01.run();
//		t02.run();
